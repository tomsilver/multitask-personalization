...
class PyBulletCSPGenerator(CSPGenerator[PyBulletState, PyBulletAction]):
    ...
    def _generate_variables(
        ...
        if self._current_mission == "hand over book":
            ...
            book_grasp = CSPVariable("book_grasp", Box(-np.pi, np.pi, dtype=np.float32))

            # Choose a handover position. Relative to the resting hand position.
            handover_position = CSPVariable(
                "handover_position", Box(-np.inf, np.inf, shape=(3,), dtype=np.float32)
            )
            ...
            grasp = CSPVariable("grasp", Box(-np.pi, np.pi, dtype=np.float32))
            ...
            if obs.held_object is not None:
                ...
                # we'll need to determine a placement for it as part of the CSP.
                (
                    first_placement_vars,
                    first_placement_init,
            ...
            robot_state = CSPVariable(
                ...
                Tuple(
                    [
                        ...
                        Box(
                            np.array(
                                self._sim.robot.joint_lower_limits, dtype=np.float32
                            ),
                            np.array(
                                self._sim.robot.joint_upper_limits, dtype=np.float32
                            ),
                            dtype=np.float32,
                        ),
                    ]
