...
class PyBulletHandoverMDP(MDP[_HandoverState, _HandoverAction]):
    ...
    @property
    def state_space(self) -> set[_HandoverState]:
        raise NotImplementedError("State space too large to enumerate")

    @property
    def action_space(self) -> set[_HandoverAction]:
        raise NotImplementedError("Action space too large to enumerate")
    ...
    def get_initial_state_distribution(
        self,
    ) -> CategoricalDistribution[_HandoverState]:
...
class PyBulletHandoverIntakeProcess(IntakeProcess[_HandoverIntakeObs, _HandoverIntakeAction]):
    ...
    @property
    def observation_space(self) -> set[_HandoverIntakeObs]:
        return {True, False}

    @property
    def action_space(self) -> set[_HandoverIntakeAction]:
        raise NotImplementedError("Action space too large to enumerate")
